...
jQuery(".add-another-servicio").click(function (e) {
  ...
  var newLi = jQuery(
    '<tr class="servicio-agregado" data-id="' + (totServicios - 1) + '"></tr>'
  ).html(newWidget);
  newLi
    .children(".valorcantidad")
    .children("input")
    .prop("required", true);
  newLi
    .children(".td-otroservicio")
    .children("input")
    .prop("required", true);
  newLi
...
$("#serviciosextra").on("click", ".remove-servicio", function (e) {
  ...
  if (idservicio > 0) {
    ...
    let servicios = document
      .getElementById("serviciosextra")
      .querySelectorAll("tr");
    servicios.forEach(fila => {
      if (fila.querySelector(".valorgrupo input").value === idservicio) {
        fila.remove();
      }
    });
  }
...
function astilleroAgregaProducto(grupoProducto, idservicio) {
  ...
  if (grupoProducto === 0) {
    ...
    var eslora =
      typeof $("#eslora").data("valor") === "undefined"
        ? 0
        : $("#eslora").data("valor");
...
function astilleroBuscaProducto(idproducto, fila) {
  ...
  var baseurl = location.href.endsWith("/")
    ? location.href
    : location.href + "/";
  ...
  $.ajax({
    ...
    success: datos => {
      ...
      fila
        .children(".valorprecio")
        .html(
          "$ " + parseFloat(datos.precio).toFixed(2) + " <small>MXN</small>"
        );
...
function recalculaPreciosProductos() {
  ...
  productos.forEach(fila => {
    astilleroBuscaProducto(
      fila.querySelector(".select-busca-producto").value,
      $(fila)
    );
  });
}
...
document.querySelectorAll(".add-servicio").forEach(service => {
  ...
  service.addEventListener("click", e => {
    ...
    var row = document.getElementById(
      `appbundle_astillerocotizacion_acservicios_${servicesLength}_servicio`
    ...
    let dollar = document.getElementById("appbundle_astillerocotizacion_dolar")
      .value;
    ...
    document.getElementById(
      `appbundle_astillerocotizacion_acservicios_${servicesLength}_cantidad`
    ).value = length;
    document
      .getElementById(
        `appbundle_astillerocotizacion_acservicios_${servicesLength}_cantidad`
      )
      .parentNode.setAttribute("data-valor", length);
    document.getElementById(
      `appbundle_astillerocotizacion_acservicios_${servicesLength}_servicio`
    ).value = id;
    document.getElementById(
      `appbundle_astillerocotizacion_acservicios_${servicesLength}_tipoCantidad`
    ).value = length;
    document
      .getElementById(
        `appbundle_astillerocotizacion_acservicios_${servicesLength}_promedio`
      )
      .setAttribute("data-dias_descuento", daysDiscount);
    row
      .querySelector(".valorpromedio")
      .setAttribute("data-dias_descuento", daysDiscount);
...
function coleccionContratistaODT(
  e,
  objeto,
  descripcion,
  cantidad,
  ...
  $(
    "#appbundle_ordendetrabajo_contratistas_" + (totProveedor - 1) + "_preciovv"
  ).val((total / 100).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,"));
...
function astilleroOcultaMuestraFila(elementId, rowId) {
  ...
  else document.getElementById(rowId).classList.add("hidden");
...
function calculaTotalesAstillero() {
  ...
  const descuentoPorcentaje = Number(
    $("#appbundle_astillerocotizacion_descuento").val()
  );
  ...
  list.forEach(tr => {
    ...
    if (!tr.classList.contains("hidden")) {
      ...
      valorSubtotal = Number(
        tr
          .querySelector(".valorsubtotal")
          .innerHTML.match(pricePattern)[0]
          .replace(",", "")
      );
      valorIva = Number(
        tr
          .querySelector(".valoriva")
          .innerHTML.match(pricePattern)[0]
          .replace(",", "")
      );
      valorTotal = Number(
  ...
  $("#gransubtot").html(
    granSubtotalAd.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
  );
  $("#grandescuento").html(
    granDescuentoAd.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
  );
  $("#graniva").html(
    granIvaAd.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
  );
  $("#grantot").html(
    granTotalAd.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,")
  );
}

/* Astillero -> Servicios Básicos -> Días Adicionales */
function astilleroDiasAdicionalesCantidad(dias_adicionales) {
  var nuevo_dias_adicionales_cantidad =
    dias_adicionales * $("#dia_adicional_cantidad").data("eslora");
...
function getIdService(id) {
  return `appbundle_astillerocotizacion_acservicios_${id}_estatus`;
}
 ...
 */
$("#appbundle_astillerocotizacion_fechaLlegada").on("change", () =>
  calculaDiasEstadiaAstillero()
);
 ...
 */
$("#appbundle_astillerocotizacion_fechaSalida").on("change", () =>
  calculaDiasEstadiaAstillero()
);
...
$("#appbundle_astillerocotizacion_dolar").keyup(() => {
  let precio = 0;
  let dolar = document.getElementById("appbundle_astillerocotizacion_dolar")
    .value;
  document.querySelector(".dolarval").innerHTML = dolar;

  /* Si existe el elemento de estadía, lo altera */
  if (document.getElementById("estadia_precio")) {
    let estadiaPrecio = document.getElementById(
      "appbundle_astillerocotizacion_acservicios_1_precio"
    ).value;
    let estadiaPrecio_mxn = estadiaPrecio * dolar;
    document.getElementById("estadia_precio").dataset[
      "valor"
    ] = estadiaPrecio_mxn;

    /* Recalcula los totales de estadía */
    calculaSubtotalesAstillero($("#fila_estadia"));
  }

  /* Si existe el elemento de días adicionales, lo altera */
  if (document.getElementById("fila_dia_adicional")) {
    let diasAdicionales = document.getElementById(
      "#appbundle_astillerocotizacion_acservicios_8_precio"
    ).value;
    let diasAdicionalesMxn = diasAdicionales * dolar;
    $("#dia_adicional_precio").data("valor", diasAdicionalesMxn);

    /* Recalcula los totales de días adicionales */
    calculaSubtotalesAstillero($("#fila_dia_adicional"));
  }
  ...
  $("#serviciosextra .servicio-agregado").each(function () {
    ...
    if (divisa === "USD") {
      ...
      $(this)
        .children(".valorprecio")
        .data("valor", preciomxn.toFixed(2));
    }
...
$("#appbundle_astillerocotizacion_acservicios_8_precio").keyup(function () {
  ...
  $("#dia_adicional_precio").html(
    dias_adicionales_precio + " <small>USD</small>"
  );
  calculaSubtotalesAstillero($("#fila_dia_adicional"));
});
